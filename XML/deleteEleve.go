...
// These hundle function remove a student create by a user in the BDD
func DeleteEleve(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method != http.MethodPost {
		// Write someone comming to the deleteEleve route in the log file
		file, _ := os.OpenFile(structure.App.LogPath[1:], os.O_APPEND|os.O_WRONLY, os.ModeAppend)
		file.WriteString(fmt.Sprintf("%s [%s] %s - DeleteEleve XML - %s\n", time.Now().Format("2006-01-02 15:04:05"), r.RemoteAddr, r.Method, teacher.Id))
		file.Close()
		return
	}

	// Get the student id from the body of the request
	...
	// Write someone comming to the deleteEleve route in the log file
	file, _ := os.OpenFile(structure.App.LogPath[1:], os.O_APPEND|os.O_WRONLY, os.ModeAppend)
	file.WriteString(fmt.Sprintf("%s [%s] %s - DeleteEleve XML - %s - datas: %s\n", time.Now().Format("2006-01-02 15:04:05"), r.RemoteAddr, r.Method, teacher.Id, id))
	file.Close()

	// Remove the student from the BDD
	err = controller.Delete("students", []string{"id", "teacherId"}, id, teacher.Id)
	if err != nil {
		// Write someone comming to the deleteEleve route in the log file
		file, _ := os.OpenFile(structure.App.LogPath[1:], os.O_APPEND|os.O_WRONLY, os.ModeAppend)
		file.WriteString(fmt.Sprintf("%s [%s] %s - DeleteEleve XML - %s - error: %s\n", time.Now().Format("2006-01-02 15:04:05"), r.RemoteAddr, r.Method, teacherId, err))
