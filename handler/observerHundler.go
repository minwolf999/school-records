...
// These hundle function the observer page
func ObserverHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method != http.MethodPost {
		err = structure.Tpl.ExecuteTemplate(w, "observer.html", yearList)
		if err != nil {
			// Write someone comming to the observer route in the log file
			file, _ := os.OpenFile(structure.App.LogPath[1:], os.O_APPEND|os.O_WRONLY, os.ModeAppend)
			file.WriteString(fmt.Sprintf("%s [%s] %s - /observer Route - %s - error: %s\n", time.Now().Format("2006-01-02 15:04:05"), r.RemoteAddr, r.Method, id, err))
